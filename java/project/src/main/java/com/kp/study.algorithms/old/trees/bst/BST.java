...
public class BST<T> {
    ...
    public TreeNode<T> findLCA(TreeNode<T> root, T left, T right) {
        if (root == null) {
            ...
            if ((root.lessThan(left) && root.greaterThan(right)) || (root.greaterThan(left) && root.lessThan(
                    right))) {
                return root;
            }
            else {
    ...
    public boolean isBST(TreeNode<T> root, T min, T max) {
        if (root == null) {
            ...
            if ((root.lessThan(max) || root.equalTo(max)) && (root.greaterThan(min) || root.equalTo(min))) {
    ...
    public boolean isInorderBST(TreeNode<T> root, T prev) {
        if (root == null) {
            ...
            return isInorderBST(root.left, prev) && root.greaterThan(prev) && isInorderBST(root.right,
                    root.key);
        }
    }
    ...
    public TreeNode<T> sortedArrayToBST(List<T> sortedArray) {
        if (sortedArray.isEmpty()) {
            ...
            if (sortedArray.size() == 1) {
                ...
                node.right = sortedArrayToBST(sortedArray.subList(mid + 1, sortedArray.size()));
            }
    ...
    public TreeNode<T> findKthSmallest(
            TreeNode<T> root, Integer k, List<T> counter) {
        if (root == null) {
            return null;
        }
        else {
